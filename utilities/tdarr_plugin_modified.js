...
const details = () => {
  return {
    ...
    Inputs: [{
      ...
      tooltip: `Specify the maximum bitrate at which this plugin will run. Files will at most use this bitrate` +
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  if (stream.codec_type == `video`) {
    ...
    if (stream.codec_name !== `mjpeg`) {
      ...
      if(fileResolution == `4KUHD`){
        bitrateMultiplier = 4;
      } else if(fileResolution == `1080p`){
        bitrateMultiplier = 2;
      } else if(fileResolution == `720p`){
        bitrateMultiplier = 1;
      }

      if (inputs.minBitrate !== ``){
        var lowerLimit = inputs.minBitrate * bitrateMultiplier;
      }

      if (inputs.maxBitrate !== ``){
        var upperLimit = inputs.maxBitrate * bitrateMultiplier;
      }

      currentBitrate = ~~(file.file_size / (duration * 0.0075));

      if (stream.codec_name == `hevc`) {
        var currentHevcBitrate = currentBitrate / 2.1;

        if (inputs.maxBitrate !== `` && currentHevcBitrate <= upperLimit) {
            response.infoLog += `☒ Input file "${stream.codec_name} (${fileResolution}, ${bitrateMultiplier})" bitrate ${currentHevcBitrate} is lower than the minimum ` +
            `bitrate threshold of ${upperLimit}. Skipping this plugin.\n`;
            videoProcessingRequired = false;
            return response;
        }
      }else{
        if (inputs.minBitrate !== `` && currentBitrate <= lowerLimit) {
            response.infoLog += `☒ Input file "${stream.codec_name} (${fileResolution}, ${bitrateMultiplier})" bitrate ${currentBitrate} is lower than the minimum ` +
            `bitrate threshold of ${lowerLimit}. Skipping this plugin.\n`;
            videoProcessingRequired = false;
            return response;
        }
      }
      ...
      if (inputs.minBitrate !== `` && currentBitrate <= lowerLimit) {
        targetBitrate=lowerLimit / 2;
      }
      
      if (inputs.maxBitrate !== `` && targetBitrate >= upperLimit) {
        targetBitrate=upperLimit;
      }

      minimumBitrate = ~~(targetBitrate * 0.7);
      maximumBitrate = ~~(targetBitrate * 1.3);

      response.infoLog += `☒ Video stream ${stream.index} is "${stream.codec_name} (${fileResolution}, ${bitrateMultiplier})", transcode required.\n `;
  ...
  if (videoProcessingRequired) {
    response.infoLog += `☑ Stream analysis complete, processing required. Current Bitrate: ${currentBitrate}, new Bitrate: ${targetBitrate}\n `;
